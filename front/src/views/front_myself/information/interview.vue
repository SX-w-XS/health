<template>
  <div class="content">
    <el-tabs v-model="activeName" @tabComplete="switchToTab">
      ...
      >    <User_contact  @go-to-next-tab="goToNextTab"> </User_contact></el-tab-pane
      >
      <el-tab-pane label="语音录入" name="audio" style="width: 100%">
        <Audio
          ...
          :dataSource="questions_audio"
          :user="now_user"
          :tablist="tab_list"
          :muilt_type="muilt_type"
        ></Audio>
      </el-tab-pane>
...
export default {
  ...
  created() {
    console.log("interview数据获取。。。。。。。。。")
      this.getRequest(modelApi).then(res => {
        if (res) {
          this.questions_audio = res.questions_audio
          this.questions_video = res.questions_video
    ...
    // //
    // this.questions_audio = jsondata.questions_audio;
    // this.questions_video = jsondata.questions_video;
    // this.user = this.$store.getters.register_user;
    // console.log("打印this.user");
    // console.log(this.user);
    // console.log("数据获取完成....打印数据");
    // console.log(this.questions_audio);
    // console.log(this.questions_video);
  },
  mounted() {},
  components: {

...
const modelApi = '/front_user/model_front_users'
...
export default {
  ...
  methods: {
    ...
    addEntity() {
      console.log(this.$store.state.register_user)
      if (this.entity.username) {
        this.postRequest(modelApi, this.entity).then(resp => {
          if (resp) {
            //注册用户成功，使用vuex保存当前用户信息
            this.user_id = resp.id
            console.log("注册用户到store")
            console.log("打印当前用户信息：",resp)
            this.$store.commit('front_user/SET_register_user',resp)

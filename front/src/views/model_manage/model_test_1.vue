  ...
  <div>
    ...
    <el-row>
      ...
      <el-col :span="10" :offset="2">
        <el-row>
          <el-input placeholder="输入用户ID查诊断分布概率" :disabled="inputIdDisable" v-model="selectedLabelId"
        ...
        <!--  单次测试分布饼状图-->
        <div class="title-container">
          <p class="test-p">单用户诊断结果 (ID:{{ SingleId }})</p>
        </div>
        <div class="test-label-charts" ref="oneLabelChart"></div>

        <div class="title-container">
          <p class="test-p">全部数据统计 (总数:{{ testCount }})</p>
        </div>
        <!--      总分布饼状图-->
  ...
  methods: {
    ...
    async testMultiple(count) {
      ...
      await getModelTrain_axios(host + modelApi + 'model_test_all' + encodeQueryParams(this.model_test_params)).then(res => {
        ...
        this.SingleId = this.rateList[0].id //标签饼图id和第一条结果同步
    ...
    huatu(count) {
      if (true) {
        ...
        this.SingleId = this.rateList[0].id //单标签饼图id和第一条结果同步
      ...
      //   //   const obj = { value: item, name: '标签' + i }
      //   //   mapArray.push(obj)
      //   // }
      //   let labelInfo = this.test_result.test_info.label_count_info
      //   let labelList = Object.keys(labelInfo).map(key => ({
